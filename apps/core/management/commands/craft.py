...
# VERSION FINALE (Contrôleur-Service + GraphQL Read)
...
class {model_name}Query(graphene.ObjectType):
    ...
    all_{pluralize(model_name.lower())} = graphene.List({model_name}Type)
    ...
    def resolve_all_{pluralize(model_name.lower())}(root, info, **kwargs):
    ...
    def handle(self, *args, **opt):
        ...
        if cmd == 'graphene:query':
            ...
            if not file.exists():
                ...
                self.stdout.write(self.style.SUCCESS(f"✔ Graphene Query créée"))
            return
